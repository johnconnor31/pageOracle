    ...
    <Box className={`marketing-page ${page ? 'browser-mode' : ''}`}>
      <Container maxWidth="xl" className="marketing-container">
        <Box component="header" className="site-nav">
          ...
          <Stack direction="row" spacing={2} alignItems="center" className="nav-actions" style={{ marginLeft: 'auto' }}>
        ...
        <Box component="main" id="top" className="hero-section">
          ...
          <Stack component="form" direction="row" onSubmit={handleSubmit} noValidate className="signup-form">
            <TextField
              ...
              fullWidth
            />
            <Button type="submit" variant="contained" className="dark-button start-button" disabled={loading}>
              {loading ? 'Fetching…' : 'Start now'}
            </Button>
          </Stack>

          {page ? (
            <Box className="browser-workspace" aria-label="Fetched page text">
              <Box className="browser-toolbar">
                <span className="browser-secure">●</span>
                <Typography noWrap>{page.url}</Typography>
              </Box>
              ...
              <Box
                ...
                sx={{
                  position: 'relative',
                  textAlign: 'left',
                  '& h1, & h2, & h3, & h4, & h5, & h6': { color: '#101827', lineHeight: 1.25, margin: '1.5rem 0 .75rem' },
                  '& p': { color: '#42536f', fontSize: '18px', lineHeight: 1.8, margin: '0 0 1.25rem' },
                  '& li': { color: '#42536f', lineHeight: 1.7, margin: '.5rem 0' },
                  '& blockquote': { borderLeft: '4px solid #0878ee', color: '#52627d', fontStyle: 'italic', margin: '1.5rem 0', padding: '.75rem 1rem' },
              ...
              {selectionPosition && (
                <Box className="ai-popover" sx={popoverSx}>
                  <Stack direction="row" justifyContent="space-between" alignItems="center">
                    <Typography fontWeight={700} fontSize="1.5rem">Ask AI</Typography>
                    <Button size="small" onClick={closeAsk}>Close</Button>
                  </Stack>
                  ...
                  <Box className="conversation">
                    {conversation.map((item, index) => (
                      <Box key={`${item.role}-${index}`} className={`chat-message ${item.role}`}>
                        <Typography>{item.text}</Typography>
                      </Box>
                    ))}
                  </Box>

                  <Box component="form" onSubmit={askAi} className="chat-form">
                    <TextField
                      value={question}
            ...
            <Box className="product-preview" aria-label="pageOracle product preview">
              ...
              <Box className="preview-content">
                <Box className="preview-article">
                  ...
                  <Typography>Fetch public page text, find the key ideas, and ask questions about it.</Typography>
                </Box>
                <Box className="assistant-card">
                  ...
                  <Typography>Highlight text after loading a page to start a conversation.</Typography>
                </Box>
              </Box>
            </Box>
          )}
