...
export default function HomePage() {
  ...
  useEffect(() => {
    const clearSelectionMenu = (event) => {
      if (!contentRef.current?.contains(event.target)) setSelectionPosition(null);
    };
    document.addEventListener('mousedown', clearSelectionMenu);
    return () => document.removeEventListener('mousedown', clearSelectionMenu);
  }, []);

  function getSelectionData() {
    const currentSelection = window.getSelection();
    if (!currentSelection || currentSelection.isCollapsed || !contentRef.current?.contains(currentSelection.anchorNode)) return null;
    ...
    return {
      ...
      position: {
        top: Math.max(8, range.bottom - container.top + 6),
  ...
  function handleContextMenu(event) {
    ...
    setSelectionPosition(selected.position);
  }
