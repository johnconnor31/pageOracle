...
export default function HomePage() {
  ...
  useEffect(() => {
    const handleSelection = () => {
      ...
      setSelectionPosition({
        top: range.bottom - container.top + 10,
        left: Math.max(8, range.left - container.left),
      });
    };
  ...
  async function handleSubmit(event) {
    ...
    setUrl(normalizedUrl); setError(''); setLoading(true); setConversation([]); setSelectionPosition(null);
    try {
  ...
  async function askAi(event) {
    ...
    try {
      const response = await fetch('/api/chat', {
        method: 'POST',
        headers: { 'Content-Type': 'application/json' },
        body: JSON.stringify({ selection, question: askedQuestion, pageTitle: page?.title, pageUrl: page?.url }),
      });
      const result = await response.json();
      ...
      setConversation((items) => [...items, { role: 'error', text: requestError.message }]);
    } finally {
  ...
  const popoverSx = {
    position: 'absolute',
    top: selectionPosition?.top + 42,
    left: selectionPosition?.left,
    zIndex: 4,
    width: { xs: 'calc(100% - 16px)', sm: 600, md: 780 },
    ...
    borderRadius: '14px',
    boxShadow: '0 16px 40px rgba(31, 55, 90, 0.24)',
          ...
          {page ? (
            <Box className="browser-workspace" aria-label="Fetched page text">
              ...
              <Box ref={contentRef} className="extracted-page" sx={{ position: 'relative', textAlign: 'left', '& h1, & h2, & h3, & h4, & h5, & h6': { color: '#101827', lineHeight: 1.25, margin: '1.5rem 0 .75rem' }, '& p': { color: '#42536f', fontSize: '18px', lineHeight: 1.8, margin: '0 0 1.25rem' }, '& li': { color: '#42536f', lineHeight: 1.7, margin: '.5rem 0' }, '& blockquote': { borderLeft: '4px solid #0878ee', color: '#52627d', fontStyle: 'italic', margin: '1.5rem 0', padding: '.75rem 1rem' }, '& img': { maxWidth: '100%', height: 'auto' } }}>
                ...
                {selectionPosition && <Button className="ask-ai-button" variant="contained" onMouseDown={(event) => event.preventDefault()} onClick={openAsk} sx={{ position: 'absolute', top: selectionPosition.top, left: selectionPosition.left, zIndex: 3 }}>Ask AI</Button>}
              </Box>
              {selectionPosition && <Box className="ai-popover" sx={popoverSx}>
                <Stack direction="row" justifyContent="space-between" alignItems="center"><Typography fontWeight={700} fontSize="1.5rem">Ask AI</Typography><Button size="small" onClick={closeAsk}>Close</Button></Stack>
                <Typography className="selection-preview">“{selection}”</Typography>
                <Box className="conversation">{conversation.map((item, index) => <Box key={`${item.role}-${index}`} className={`chat-message ${item.role}`}><Typography>{item.text}</Typography></Box>)}</Box>
                <Box component="form" onSubmit={askAi} className="chat-form"><TextField value={question} onChange={(event) => setQuestion(event.target.value)} placeholder="Ask a follow-up…" size="small" fullWidth /><Button type="submit" disabled={chatLoading || !question.trim()}>{chatLoading ? '…' : 'Send'}</Button></Box>
              </Box>}
            </Box>
          ) : (
            <Box className="product-preview" aria-label="pageOracle product preview"><Box className="preview-toolbar"><span /><span /><span /></Box><Box className="preview-content"><Box className="preview-article"><Typography className="preview-kicker">READING CONTEXT</Typography><Typography component="h2">Turn every page into a conversation.</Typography><Typography>Fetch public page text, find the key ideas, and ask questions about it.</Typography></Box><Box className="assistant-card"><Typography className="preview-kicker">PAGEORACLE AI</Typography><Typography component="h3">What does this mean?</Typography><Typography>Highlight text after loading a page to start a conversation.</Typography></Box></Box></Box>
          )}
        </Box>
      </Container>
    </Box>
